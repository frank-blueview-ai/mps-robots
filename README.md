...
`POST /bridge/home` sends ORA to the same final home joint coordinates used by the classroom Python program: `Joints(0, 0, 0, 0, 0, 0)`.
...
- `Station Actions`: emergency stop, set ready, initial position/home (`J1`-`J6` all `0`), gripper, and compact pose summary.
...
- Lite6 mesh geometry styled to match the ORA product arm, with white shells, gray collars, yellow joint accents, and gripper;
