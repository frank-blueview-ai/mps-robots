...
import { CSSProperties, ChangeEvent, FormEvent, KeyboardEvent as ReactKeyboardEvent, PointerEvent as ReactPointerEvent, ReactNode, useCallback, useEffect, useMemo, useRef, useState } from "react";
...
const visualHomeStorageKey = "ora-visual-home-joint-offsets";
const zeroVisualJointOffsets = [0, 0, 0, 0, 0, 0];
...
function loadVisualJointOffsets() {
  try {
    const parsed = JSON.parse(window.localStorage.getItem(visualHomeStorageKey) || "[]");
    if (!Array.isArray(parsed) || parsed.length < 6) return zeroVisualJointOffsets;
    return zeroVisualJointOffsets.map((_, index) => {
      const value = Number(parsed[index]);
      return Number.isFinite(value) ? value : 0;
    });
  } catch {
    return zeroVisualJointOffsets;
  }
}

function saveVisualJointOffsets(offsets: number[]) {
  window.localStorage.setItem(visualHomeStorageKey, JSON.stringify(offsets.slice(0, 6)));
}
...
function StationApp() {
  ...
  const [visualJointOffsets, setVisualJointOffsets] = useState(loadVisualJointOffsets);
  ...
  const persistVisualJointOffsets = useCallback(() => {
    saveVisualJointOffsets(visualJointOffsets);
    recordEvent("ok", "3D visual home alignment saved");
  }, [recordEvent, visualJointOffsets]);
  ...
  async function sendBridgeCommand(action: string, data: unknown = {}) {
    if (!liveControl && !["stop", "ready", "clear-error", "move-step-over"].includes(action)) {
      setCommandMessage("Controls locked");
      recordEvent("warning", "Command blocked: live control is disabled");
      return;
    }

    if (!safeToMove && !["stop", "ready", "clear-error", "move-step-over"].includes(action)) {
      ...
      case "manual":
        return (
          <ManualControl
            ...
            visualJointOffsets={visualJointOffsets}
            onClearEvents={clearEvents}
            onPreviewJoystickIntent={setJoystickIntent}
            onClearJoystickIntent={clearJoystickIntent}
            onVisualJointOffsetsChange={setVisualJointOffsets}
            onSaveVisualJointOffsets={persistVisualJointOffsets}
...
function ModelPopout() {
  ...
  const [visualJointOffsets] = useState(loadVisualJointOffsets);
...
function ManualControl(props: {
  ...
  onVisualJointOffsetsChange: (offsets: number[]) => void;
  onSaveVisualJointOffsets: () => void;
  onToggleLive: () => void;
  ...
  const [controlPanelPercent, setControlPanelPercent] = useState(() => {
    const saved = Number(window.localStorage.getItem("ora-control-panel-percent"));
    return Number.isFinite(saved) ? clampPercent(saved) : 42;
  });
  const [modelFollowsLive, setModelFollowsLive] = useState(true);
  const [cameraLocked, setCameraLocked] = useState(false);
  const [resetCameraSignal, setResetCameraSignal] = useState(0);
  const [calibratingModel, setCalibratingModel] = useState(false);
  ...
  const errorCode = props.bridge.latest?.xarm_error_code ?? 0;
  const canClearError = props.bridge.connected && errorCode > 0;
  ...
  const jointStepDegrees = stepSize === "large" ? 5 : stepSize === "medium" ? 2 : 0.5;
  ...
  async function startContinuousStep(direction: string, intent: JoystickIntent) {
    if (!controlsEnabled || activeStepRef.current === direction) return;
  ...
  async function stopContinuousStep(direction: string) {
    if (activeStepRef.current !== direction) return;
  ...
  async function finiteStep(direction: string, intent: JoystickIntent) {
    if (!controlsEnabled) return;
    props.onPreviewJoystickIntent(intent);
    try {
      await props.onCommand("move-step", { isLoop: false, direction, isMoveTool: false, stepSize, speedScale: speedPercent / 100 });
    } finally {
      props.onClearJoystickIntent();
    }
  }

  async function jointStep(jointIndex: number, deltaSign: -1 | 1) {
    if (!controlsEnabled) return;
    await props.onCommand("joint-step", {
      jointIndex,
      deltaDegrees: deltaSign * jointStepDegrees,
    });
  }

  function moveButtonHandlers(direction: string, intent: JoystickIntent) {
    return {
      ...
      onPointerDown: (event: ReactPointerEvent<HTMLButtonElement>) => {
        ...
        void startContinuousStep(direction, intent);
      },
      onPointerUp: (event: ReactPointerEvent<HTMLButtonElement>) => {
        ...
        void stopContinuousStep(direction);
      },
      onPointerCancel: () => void stopContinuousStep(direction),
      onPointerLeave: () => {
        props.onClearJoystickIntent();
        void stopContinuousStep(direction);
      },
      onClick: () => {
        ...
        if (!activeStepRef.current) {
          void finiteStep(direction, intent);
        }
      },
  ...
  function setVisualJointOffset(index: number, value: number) {
    props.onVisualJointOffsetsChange(props.visualJointOffsets.map((offset, offsetIndex) => (offsetIndex === index ? value : offset)));
  }

  function nudgeVisualJointOffset(index: number, delta: number) {
    setVisualJointOffset(index, Number((props.visualJointOffsets[index] + delta).toFixed(1)));
  }

  function resetVisualJointOffsets() {
    props.onVisualJointOffsetsChange([...zeroVisualJointOffsets]);
  }
      ...
      <div className="manual-cockpit" ref={cockpitRef} style={cockpitStyle}>
        <section className="control-rail" aria-label="Control panel">
          <div className="tool-panel teach-panel">
            <div className="panel-headline">
              <div>
                <h3>Teach Pendant</h3>
                <p>Step motion with teacher-gated live control.</p>
              </div>
            ...
            <div className="movement-pad-row">
              <div className="xy-pad precision-pad" aria-label="XY movement pad">
                <button type="button" disabled={!controlsEnabled} {...moveButtonHandlers("position-y-increase", intents.yIncrease)}>Y+</button>
                <button type="button" disabled={!controlsEnabled} {...moveButtonHandlers("position-x-decrease", intents.xDecrease)}>X-</button>
                <div className="pad-center precision-center">TCP</div>
                <button type="button" disabled={!controlsEnabled} {...moveButtonHandlers("position-x-increase", intents.xIncrease)}>X+</button>
                <button type="button" disabled={!controlsEnabled} {...moveButtonHandlers("position-y-decrease", intents.yDecrease)}>Y-</button>
              </div>
              <div className="vertical-pad" aria-label="Z movement pad">
                <button type="button" disabled={!controlsEnabled} {...moveButtonHandlers("position-z-increase", intents.zIncrease)}><ChevronDown className="flip" size={18} /> Z Up</button>
                <button type="button" disabled={!controlsEnabled} {...moveButtonHandlers("position-z-decrease", intents.zDecrease)}><ChevronDown size={18} /> Z Down</button>
              </div>
            </div>

            <section className="joint-jog-panel" aria-label="Joint jog controls">
              <div className="joint-jog-head">
                <span>Joint jog</span>
                <strong>{jointStepDegrees}°</strong>
              </div>
              <div className="joint-jog-grid">
                {jointItems.map((joint, index) => (
                  <div className="joint-jog-row" key={joint.label}>
                    <span>{joint.label}</span>
                    <button type="button" disabled={!controlsEnabled} onClick={() => void jointStep(index, -1)} aria-label={`${joint.label} negative`}>-</button>
                    <strong>{joint.value}</strong>
                    <button type="button" disabled={!controlsEnabled} onClick={() => void jointStep(index, 1)} aria-label={`${joint.label} positive`}>+</button>
                  </div>
                ))}
              </div>
            </section>
          ...
          <div className="tool-panel action-panel">
            ...
            <div className="button-stack">
              ...
              <button type="button" disabled={!canClearError} onClick={() => void props.onCommand("clear-error")}><RefreshCw size={18} /> Clear Error</button>
        ...
        <section className="model-rail" aria-label="3D model panel">
          <div className="model-toolbar">
            ...
            <button type="button" aria-pressed={calibratingModel} onClick={() => setCalibratingModel((value) => !value)}>
              <Settings size={18} /> Calibrate Home
            </button>
          ...
          {calibratingModel && (
            <section className="visual-calibration-panel" aria-label="3D home calibration">
              <div className="panel-headline">
                <div>
                  <h3>3D Home Alignment</h3>
                  <p>Adjust the model only; the physical arm does not move.</p>
                </div>
                <div className="action-row">
                  <button type="button" onClick={resetVisualJointOffsets}><RefreshCw size={16} /> Reset</button>
                  <button type="button" onClick={props.onSaveVisualJointOffsets}><Save size={16} /> Save Home</button>
                </div>
              </div>
              <div className="calibration-grid">
                {props.visualJointOffsets.map((offset, index) => (
                  <label key={index} className="calibration-control">
                    <span>J{index + 1}</span>
                    <button type="button" aria-label={`Decrease visual J${index + 1}`} onClick={() => nudgeVisualJointOffset(index, -1)}>-</button>
                    <input
                      aria-label={`Visual J${index + 1} offset`}
                      type="range"
                      min="-180"
                      max="180"
                      step="0.5"
                      value={offset}
                      onChange={(event) => setVisualJointOffset(index, Number(event.target.value))}
                    />
                    <button type="button" aria-label={`Increase visual J${index + 1}`} onClick={() => nudgeVisualJointOffset(index, 1)}>+</button>
                    <strong>{offset.toFixed(1)}°</strong>
                  </label>
                ))}
              </div>
            </section>
          ...
          <ArmVisualization
            ...
            cameraLocked={cameraLocked}
            resetCameraSignal={resetCameraSignal}
            visualJointOffsets={props.visualJointOffsets}
          />
        </section>
      </div>
