...
test("local bridge exposes connected ORA status and read-only command path", async ({ request }) => {
  const status = await request.get("http://localhost:8081/bridge/status", { timeout: 10_000 }).catch(() => null);
  ...
  const command = await request.post("http://localhost:8081/bridge/command", {
    data: {
